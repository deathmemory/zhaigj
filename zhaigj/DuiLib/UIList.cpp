...
SIZE CListBodyUI::EstimateSize(SIZE szAvailable)  //add by lyb
{
	m_cxyFixed = szAvailable;
	return m_cxyFixed;
}
/////////////////////////////////////////////////////////////////////////////////////
//
...
CListLabelElementUI::~CListLabelElementUI()
{
	...
	{
			ElementIamges* pImg = static_cast<ElementIamges*>(m_aImages[i]);
		if (pImg)
		{
			delete pImg;
		}
...
void CListLabelElementUI::DoPaint(HDC hDC, const RECT& rcPaint)
{
	...
	DrawItemImage(hDC,m_rcItem);
    DrawItemText(hDC, m_rcItem);
}

void CListLabelElementUI::DrawItemImage(HDC hDC,const RECT& rcItem)
{
	if( m_pOwner == NULL ) return;
	TListInfoUI* pInfo = m_pOwner->GetListInfo();
	...
	for( int i = 0; i < pInfo->nColumns; i++ )
	{
		...
		ElementIamges* pImg = static_cast<ElementIamges*>(m_aImages[i]);
		if (!pImg)continue;
		if (!pImg->strImage.IsEmpty())
		{
			CRenderEngine::DrawImageString(hDC,m_pManager,rcItem,rcItem,pImg->strImage,0,pImg->strModify);
		}
		else
			CRenderEngine::DrawImageString(hDC,m_pManager,rcItem,rcItem,NULL,pImg->uImageID,pImg->strModify);

	}
}

UINT CListLabelElementUI::GetImage(int iIndex) const
{
	ElementIamges* pImg = static_cast<ElementIamges*>(m_aImages[iIndex]);
	if (pImg)
	{
		return pImg->uImageID;
	}
	return 0;
}

void CListLabelElementUI::SetImage(int iIndex, LPCTSTR sImage,UINT uImage,LPCTSTR sModify)
{
	if( m_pOwner == NULL ) return;
	TListInfoUI* pInfo = m_pOwner->GetListInfo();
	if( iIndex < 0 || iIndex >= pInfo->nColumns ) return;
	while( m_aImages.GetSize() < pInfo->nColumns ) { m_aImages.Add(NULL); }

	ElementIamges* pImg = static_cast<ElementIamges*>(m_aImages[iIndex]);
	//if( (pImg == NULL )) return;
	if( pImg ) delete pImg;

	m_aImages.SetAt(iIndex, new TagElementIamges(uImage,sImage,sModify));
	Invalidate();
}
...
CListTextElementUI::~CListTextElementUI()
{
    ...
    for( int it = 0; it < m_aTexts.GetSize(); it++ ) {
        pText = static_cast<CString*>(m_aTexts[it]);
...
{
    CString* pText = static_cast<CString*>(m_aTexts.GetAt(iIndex));
...
void CListTextElementUI::SetText(int iIndex, LPCTSTR pstrText)
{
    ...
    CString* pText = static_cast<CString*>(m_aTexts[iIndex]);
    if( (pText == NULL && pstrText == NULL) || (pText && *pText == pstrText) ) return;

    if( pText ) delete pText;
    m_aTexts.SetAt(iIndex, new CString(pstrText));
    Invalidate();
}
...
CString* CListTextElementUI::GetLinkContent(int iIndex)
{
...
void CListContainerElementUI::DoEvent(TEventUI& event)
{
    ...
    if( event.Type == UIEVENT_MOUSEMOVE )
    {
		m_pManager->SendNotify(this, _T("list_Container_mousemove"));
        return;
    }
    if( event.Type == UIEVENT_MOUSEENTER )
    {
    ...
    if( event.Type == UIEVENT_MOUSELEAVE )
    {
		m_pManager->SendNotify(this, _T("list_Container_mouseleave"));
        if( (m_uButtonState & UISTATE_HOT) != 0 ) {
            m_uButtonState &= ~UISTATE_HOT;
            Invalidate();
        }
