...
CLabelUI::CLabelUI() : m_uTextStyle(DT_VCENTER), m_dwTextColor(0), 
    m_dwDisabledTextColor(0), m_iFont(-1), m_bShowHtml(FALSE),m_bLabelClick(FALSE)
{
...
{
	return  (IsEnabled()&&m_bLabelClick ? UIFLAG_SETCURSOR : 0);
}
...
{
	m_bLabelClick = bClick;
}

void CLabelUI::SetImage(UINT nImage)
{
	SetBkImage(nImage);
}

void CLabelUI::SetImage(LPCTSTR sImage)
{
	SetBkImage(sImage);
}

void CLabelUI::SetImageModify(LPCTSTR sImageModify)
{
    m_sImageModify = sImageModify;
}
...
void CLabelUI::PaintBkImage(HDC hDC)
{
	...
	{
		if( !DrawImage(hDC, (LPCTSTR)m_sBkImage,m_sImageModify) ) m_sBkImage.Empty();
	}
	else if (m_nBkImage!=0)
	{
		CString strModify;
		if (!m_sResType.IsEmpty())
		{
			strModify.Format(_T("%s restype='%s'"),(LPCTSTR)m_sImageModify,m_sResType);
			if(!DrawImage(hDC, m_nBkImage,strModify)) m_nBkImage = 0;
		}
		else
		{
			if(!DrawImage(hDC, m_nBkImage,m_sImageModify)) m_nBkImage = 0;
		}
	}
}
...
void CLabelUI::SetVisible(BOOL bVisible )
{
	...
	if (!bVisible)
	{
		if (m_bMouseMoveNotify)
		{
			if(m_pManager)m_pManager->SendNotify(this,_T("label_mouseleave"),(WPARAM)this,(LPARAM)GetTag());
		}
	}
}
...
void CLabelUI::DoEvent(TEventUI& event)
{
	...
	if( event.Type == UIEVENT_MOUSEMOVE )
	{
		...
		{
			m_bMouseMoved = TRUE;
			if(m_pManager)m_pManager->SendNotify(this,_T("label_mousemove"),(WPARAM)event.pSender,event.pSender->GetTag());
		}
		if (IsParentNotify())
		{
			if (m_pParent)
    ...
    if( event.Type == UIEVENT_MOUSELEAVE )
    {
		...
		{
			m_bMouseMoved = FALSE;
			if(m_pManager)m_pManager->SendNotify(this,_T("label_mouseleave"),(WPARAM)event.pSender,event.pSender->GetTag());
		}
	...
	if( event.Type == UIEVENT_MOUSEHOVER)
	{
		if (m_bMouseMoveNotify)
		{
			if(m_pManager)m_pManager->SendNotify(this,_T("label_hover"),(WPARAM)event.pSender,0);
		}
		return;
	}
	if( event.Type == UIEVENT_DBLCLICK )
	{
		if (m_bMouseMoveNotify)
		{
			if(m_pManager)m_pManager->SendNotify(this,_T("label_dbclick"),(WPARAM)event.pSender,0);
		}
		return;
	}
	if( event.Type == UIEVENT_RBUTTONUP )
	{
		if (m_bMouseMoveNotify)
		{
			if(m_pManager)m_pManager->SendNotify(this,_T("label_rbuttonup"),(WPARAM)event.pSender,0);
		}
		return;
	}
...
CButtonUI::CButtonUI() : m_uButtonState(0), m_dwHotTextColor(0), m_dwPushedTextColor(0), m_dwFocusedTextColor(0),m_uButtonType(0)
{
    m_uTextStyle				 = DT_SINGLELINE | DT_VCENTER | DT_CENTER;
	m_sEightStateImageSize.cx	 = 0;
	m_sEightStateImageSize.cy	 = 0;
	m_uCheckGroupMinID			 = 0;
	m_uCheckGroupMaxID			 = 0;
	m_uImageID = 0;
}
...
BOOL CButtonUI::Activate()
{
	...
	if (m_uButtonType == BUTTON_TYPE_CHECK || m_uButtonType == BUTTON_TYPE_RADIOBOX)
	{
		SetCheck(TRUE);
	}
	else
...
void CButtonUI::SetTextColor(DWORD dNormalColor,DWORD dHotColor,DWORD dPressColor,DWORD ddisableColor)
{
	CLabelUI::SetTextColor(dNormalColor);
	SetFocusedTextColor(dNormalColor);
	SetHotTextColor(dHotColor);
	SetPushedTextColor(dPressColor);
	SetDisabledTextColor(ddisableColor);
}
...
void CButtonUI::SetCheckGroup(UINT nMin , UINT nMax)
{
	m_uCheckGroupMinID = nMin;
	m_uCheckGroupMaxID = nMax;
}

void CButtonUI::SetCheck(BOOL bIsC)
{
	CControlUI::SetCheck(bIsC);
	if (bIsC&&(m_uButtonType == BUTTON_TYPE_CHECK || m_uButtonType == BUTTON_TYPE_RADIOBOX))
	{
		if (m_uCheckGroupMinID!=0 || m_uCheckGroupMaxID>m_uCheckGroupMinID)
		{
			for (int i = m_uCheckGroupMinID ;i<= m_uCheckGroupMaxID; i++)
			{
				if(m_pManager != NULL)
				{
					CButtonUI * pBtn =dynamic_cast<CButtonUI *> (m_pManager->FindControl(i));
					if (pBtn)
					{
						if (i != GetID())
						 pBtn->SetCheck(FALSE);
...
void CButtonUI::SetImage(UINT uImageID)
{
	m_uImageID = uImageID;
	Invalidate();
}

void CButtonUI::SetModifyString(LPCTSTR pStrModify)
{
	m_sImageModify = pStrModify;
	Invalidate();
}

void CButtonUI::SetButtonType(UINT uType)
{
	m_uButtonType = uType;
}

void CButtonUI::SetEightStateImageSize(SIZE size)
{
	 m_sEightStateImageSize = size;
}
...
void CButtonUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
    ...
    if( _tcscmp(pstrName, _T("hottextcolor")) == 0 ) {
        if( *pstrValue == _T('#')) pstrValue = ::CharNext(pstrValue);
        LPTSTR pstr = NULL;
        DWORD clrColor = _tcstoul(pstrValue, &pstr, 16);
...
void CButtonUI::PaintText(HDC hDC)
{
	...
	UINT uLeftPadding = 0;
	if (m_uButtonType == BUTTON_TYPE_CHECKBOX || m_uButtonType == BUTTON_TYPE_RADIOBOX)
	{
		if (m_sEightStateImageSize.cx!=0&&m_sEightStateImageSize.cy!=0)
		{
			uLeftPadding = GetHeight() - m_sEightStateImageSize.cy + m_sEightStateImageSize.cx;
			m_uTextStyle = DT_VCENTER|DT_LEFT|DT_SINGLELINE;
	
		}
	}
    int nLinks = 0;
    RECT rc = m_rcItem;
    rc.left += m_rcTextPadding.left+uLeftPadding;
    rc.right -= m_rcTextPadding.right;
    rc.top += m_rcTextPadding.top;
    rc.bottom -= m_rcTextPadding.bottom;
	...
	if( ((m_uButtonState & UISTATE_PUSHED) != 0|| (m_uButtonType == BUTTON_TYPE_CHECK&&IsChecked() )) && (GetPushedTextColor() != 0) )
...
void CButtonUI::PaintStatusImage(HDC hDC)
{
	...
	if (m_uButtonType == BUTTON_TYPE_CHECKBOX || m_uButtonType == BUTTON_TYPE_RADIOBOX)
	{
		if (IsChecked())uTop = 50;
		else uBottom = 50;

		if (m_sEightStateImageSize.cx!=0&&m_sEightStateImageSize.cy!=0)
		{
			UINT nTopBottomGap = (GetFixedHeight() - m_sEightStateImageSize.cy)/2;
			strDest.Format(_T("dest='%d,%d,%d,%d'"),nTopBottomGap,nTopBottomGap,nTopBottomGap+m_sEightStateImageSize.cx,nTopBottomGap+m_sEightStateImageSize.cy);
		}
	}
	
    if( (m_uButtonState & UISTATE_DISABLED) != 0 ) {
		CString strModify;strModify.Format(_T("%s %s source='%d,%d,%d,%d' sourcePersent='TRUE'"),(LPCTSTR)m_sImageModify,(LPCTSTR)strDest,75,uTop,100,uBottom);
        if( !m_sImage.IsEmpty() ) {
            if( !DrawImage(hDC, (LPCTSTR)m_sImage,(LPCTSTR)strModify) ) m_sImage.Empty();
            else return;
        }
		else if (m_uImageID!=0)
		{
			
			if( !DrawImage(hDC, m_uImageID,(LPCTSTR)strModify) ) m_uImageID=0;
			else return;
		}
    }
    else if( (m_uButtonState & UISTATE_PUSHED) != 0 || ((m_uButtonType == BUTTON_TYPE_CHECK)&&IsChecked() )){
		CString strModify;strModify.Format(_T("%s %s source='%d,%d,%d,%d' sourcePersent='TRUE'"),(LPCTSTR)m_sImageModify,(LPCTSTR)strDest,50,uTop,75,uBottom);
        if( !m_sImage.IsEmpty() ) {
            if( !DrawImage(hDC, (LPCTSTR)m_sImage,(LPCTSTR)strModify) ) m_sImage.Empty();
            else return;
        }
		...
		CString strModify;strModify.Format(_T("%s %s source='%d,%d,%d,%d' sourcePersent='TRUE'"),(LPCTSTR)m_sImageModify,(LPCTSTR)strDest,25,uTop,50,uBottom);
        if( !m_sImage.IsEmpty() ) {
            if( !DrawImage(hDC, (LPCTSTR)m_sImage,(LPCTSTR)strModify) ) m_sImage.Empty();
            else return;
        }
		...
		CString strModify;strModify.Format(_T("%s %s source='%d,%d,%d,%d' sourcePersent='TRUE'"),(LPCTSTR)m_sImageModify,(LPCTSTR)strDest,0,uTop,25,uBottom);
        if( !m_sImage.IsEmpty() ) {
            if( !DrawImage(hDC, (LPCTSTR)m_sImage,(LPCTSTR)strModify) ) m_sImage.Empty();
            else return;
        }
...
CString* CTextUI::GetLinkContent(int iIndex)
{
...
SIZE CTextUI::EstimateSize(SIZE szAvailable)
{
    ...
    if( m_bShowHtml ) {   
        int nLinks = 0;
        CRenderEngine::DrawHtmlText(m_pManager->GetPaintDC(), m_pManager, rcText, m_sText, m_dwTextColor, NULL, NULL, nLinks, DT_CALCRECT | m_uTextStyle,m_nHoverLink);
    }
    else {
...
void CTextUI::PaintText(HDC hDC)
{
    ...
    if( IsEnabled() ) {
        if( m_bShowHtml )
            CRenderEngine::DrawHtmlText(hDC, m_pManager, rc, m_sText, m_dwTextColor, \
            m_rcLinks, m_sLinks, m_nLinks, m_uTextStyle,m_nHoverLink);
        else
            CRenderEngine::DrawText(hDC, m_pManager, rc, m_sText, m_dwTextColor, \
            m_iFont, m_uTextStyle);
    }
...
void CProgressUI::PaintStatusImage(HDC hDC)
{
	...
	if (m_bShowAnmi)
	{
		...
		if (m_uAsImage)
		{
			...
			if( !DrawImage(hDC, m_uAsImage, (LPCTSTR)strModify) ) m_uAsImage = 0;
		}
		if (m_uAbImage)
		{
			...
			if( !DrawImage(hDC, m_uAbImage, (LPCTSTR)depart) ) m_uAbImage = 0;
		}
	}
	else
	{
		...
		else if (m_uFgImage!=0)
		{
			...
			if( !DrawImage(hDC, m_uFgImage, (LPCTSTR)m_sFgImageModify) ) m_uFgImage = 0;
			else return;
		}
	}
}
...
void CScrollBarUI::DoEvent(TEventUI& event)
{
        ...
        if( (m_uThumbState & UISTATE_CAPTURED) != 0 ) {
            if( !m_bHorizontal ) {
                 m_nLastScrollOffset = (event.ptMouse.y - ptLastMouse.y) * m_nRange / \
                    (m_rcItem.bottom - m_rcItem.top - m_rcThumb.bottom + m_rcThumb.top - nBtnWitdh);
            }
            else {
                m_nLastScrollOffset = (event.ptMouse.x - ptLastMouse.x) * m_nRange / \
                    (m_rcItem.right - m_rcItem.left - m_rcThumb.right + m_rcThumb.left - nBtnWitdh);
            }
        }
        else {
    ...
    if(m_nLastScrollOffset!=0 )//event.Type == UIEVENT_TIMER && event.wParam == DEFAULT_TIMERID )
    {
...
UINT CScrollBarUI::GetReguleImage()
{
	return m_uImageID;
}
void CScrollBarUI::SetRegulerImage(UINT uImage,int nblockW,int nblockH)
{
	m_uImageID = uImage;
}
...
void CScrollBarUI::PaintButton1(HDC hDC)
{
    ...
    m_sImageModify.Empty();
    m_sImageModify.Format(_T("dest='%d,%d,%d,%d'"), m_rcButton1.left - m_rcItem.left, \
...
void CScrollBarUI::PaintButton2(HDC hDC)
{
    ...
    m_sImageModify.Empty();
    m_sImageModify.Format(_T("dest='%d,%d,%d,%d'"), m_rcButton2.left - m_rcItem.left, \
...
void CScrollBarUI::PaintThumb(HDC hDC)
{
    ...
    m_sImageModify.Empty();
    m_sImageModify.Format(_T("dest='%d,%d,%d,%d'"), m_rcThumb.left - m_rcItem.left, \
...
void CScrollBarUI::PaintRail(HDC hDC)
{
    ...
    if( !m_bHorizontal ) {
        ...
        m_sImageModify.Format(_T("dest='%d,%d,%d,%d'"), \
