...
void CListTextElementUI::SetText(int iIndex, LPCTSTR pstrText)
{
	...
	if ( pText ) //by cddjr 2011/10/20
		pText->Format(pstrText);
	else
		m_aTexts.SetAt(iIndex, new CDuiString(pstrText));
    Invalidate();
}

void CListTextElementUI::SetTextStyle(int iIndex,DWORD style)
{
	if( m_pOwner == NULL ) return;
	TListInfoUI* pInfo = m_pOwner->GetListInfo();
	if( iIndex < 0 || iIndex >= pInfo->nColumns ) return;
	while( m_aTextStyles.GetSize() < pInfo->nColumns ) { m_aTextStyles.Add(NULL); }

	DWORD dwTextStyle = (DWORD)static_cast<LPVOID>(m_aTextStyles[iIndex]);
	if((dwTextStyle == style) ) return;

	m_aTextStyles.SetAt(iIndex, (LPVOID)style);

	Invalidate();
}
DWORD CListTextElementUI::GetTextStyle(int iIndex)
{
	DWORD dwTextStyle = (DWORD)static_cast<LPVOID>(m_aTextStyles.GetAt(iIndex));
...
void CListTextElementUI::DrawItemText(HDC hDC, const RECT& rcItem)
{
    ...
    for( int i = 0; i < pInfo->nColumns; i++ )
    {
        ...
        if( pInfo->bShowHtml )
            CRenderEngine::DrawHtmlText(hDC, m_pManager, rcItem, strText.GetString(), iTextColor, \
                &m_rcLinks[m_nLinks], &m_sLinks[m_nLinks], nLinks, DT_SINGLELINE | GetTextStyle(i));
        else
            CRenderEngine::DrawText(hDC, m_pManager, rcItem, strText.GetString(), iTextColor, \
            pInfo->nFont, DT_SINGLELINE |GetTextStyle(i));
...
BOOL CListContainerElementUI::Select(BOOL bSelect)
{
    if( !IsEnabled() ) return FALSE;
    if( bSelect == m_bSelected ) return TRUE;
    m_bSelected = bSelect&&m_bCanSel;
    if( m_bSelected && m_pOwner != NULL ) m_pOwner->SelectItem(m_iIndex);
    Invalidate();
