...
LRESULT CComboWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    if( uMsg == WM_CREATE ) {
        ...
        m_pLayout->SetBorderColor(ARGB(255,92,131,174));
...
void CComboUI::SetStateImage(UINT nID)
{
	m_uStateImage = nID;
	Invalidate();
}
...
CString CComboUI::GetText() const
{
...
CString CComboUI::GetDropBoxAttributeList()
{
...
void CComboUI::SetStateImageModify(LPCTSTR pStrModify)
{
	m_strStateImageModify = pStrModify;
}
...
void CComboUI::PaintStatusImage(HDC hDC)
{
	...
	if( (m_uButtonState & UISTATE_DISABLED) != 0 ) {
		...
		else if (m_uStateImage!=0)
		{
			CString strModify;strModify.Format(_T("%s source='%d,%d,%d,%d' sourcePersent='TRUE'"),(LPCTSTR)m_strStateImageModify,75,0,100,100);
			if( !DrawImage(hDC, RES(m_uStateImage),(LPCTSTR)strModify) ) m_uStateImage=0;
			else return;
		}
	}
		...
		{
			CString strModify;strModify.Format(_T("%s source='%d,%d,%d,%d' sourcePersent='TRUE'"),(LPCTSTR)m_strStateImageModify,50,0,75,100);
			if( !DrawImage(hDC, RES(m_uStateImage),(LPCTSTR)strModify) ) m_uStateImage=0;
			else return;
		}
	}
		...
		{
			CString strModify;strModify.Format(_T("%s source='%d,%d,%d,%d' sourcePersent='TRUE'"),(LPCTSTR)m_strStateImageModify,25,0,50,100);
			if( !DrawImage(hDC, RES(m_uStateImage),(LPCTSTR)strModify) ) m_uStateImage=0;
			else return;
		}
	}
		...
		{
			CString strModify;strModify.Format(_T("%s source='%d,%d,%d,%d' sourcePersent='TRUE'"),(LPCTSTR)m_strStateImageModify,0,0,25,100);
			if( !DrawImage(hDC, RES(m_uStateImage),(LPCTSTR)strModify) ) m_uStateImage=0;
			else return;
		}
	}
