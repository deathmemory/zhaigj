...
public:
	...
	void PasteSpecial(UINT uClipFormat, DWORD dwAspect = 0, HMETAFILE hMF = 0)
	{
		...
		ITextServices *pServer = GetTextServices2(); 
		if (pServer)
		{
			pServer->TxSendMessage(EM_PASTESPECIAL, uClipFormat, (LPARAM)&reps, NULL);
			pServer->Release();
		}
...
void CRichEditUI::SetSelFontFormat(TFontInfo * pFont,COLORREF col)
{
	...
	if (!pFont->sFontName.IsEmpty())					// 设置字体名称
	{
		cf.dwMask = cf.dwMask | CFM_FACE;
		_tcscpy(cf.szFaceName, pFont->sFontName.GetString());
	}

	if (pFont->iSize > 0)						// 设置字体高度
	{
		cf.dwMask = cf.dwMask | CFM_SIZE;
		cf.yHeight = pFont->iSize * 20;
	}

	cf.dwMask |= CFM_COLOR | CFM_BOLD | CFM_ITALIC | CFM_UNDERLINE | CFM_LINK;

	cf.crTextColor = col;				// 设置字体颜色
	...
	if (pFont->bItalic)							// 设置倾斜
		cf.dwEffects |= CFE_ITALIC;
	else
		cf.dwEffects &= ~CFE_ITALIC;

	if (pFont->bUnderline)							// 设置下划线
		cf.dwEffects |= CFE_UNDERLINE;
	else
		cf.dwEffects &= ~CFE_UNDERLINE;
...
void CRichEditUI::SetCanPaste()
{
	 TxSendMessage(EM_CANPASTE, CF_TEXT, 0, 0);
}
...
void  CRichEditUI::SetBidioptions(BIDIOPTIONS * bidi)
{
	 TxSendMessage(EM_SETBIDIOPTIONS, 0, (LPARAM)bidi, 0);
}

BIDIOPTIONS CRichEditUI::GetBidioptions()
{
	BIDIOPTIONS bidi;
	TxSendMessage(EM_GETBIDIOPTIONS, 0, (LPARAM)&bidi, 0);
...
HRESULT CRichEditUI::TxSendMessage(UINT msg, WPARAM wparam, LPARAM lparam, LRESULT *plresult) const
{
	if( m_pTwh ) {
		...
		ITextServices *pServer = m_pTwh->GetTextServices2();
		LRESULT res = 0;
		if (pServer)
		{
			pServer->TxSendMessage(msg, wparam, lparam, plresult);
			pServer->Release();
		}
		if (msg == WM_CHAR)
		{
			if(m_pManager)m_pManager->SendNotify((CControlUI*)this,_T("edit_notify"),(WPARAM)this,(LPARAM)GetTag());
		}
		return res;
    }
    return S_FALSE;
...
void CRichEditUI::DoEvent(TEventUI& event)
{
    ...
    if( event.Type == UIEVENT_SETFOCUS ) {
		...
		if (m_pManager)
		{
			m_pManager->SendNotify(this,_T("set_focus"));
		}
		return;
    }
...
LRESULT CRichEditUI::MessageHandler(LPMSG lpMsg, BOOL& bHandled)
{
	...
	//m_pTwh->GetTextServices()->
	IRichEditOle* pEditOle = NULL;
	TxSendMessage(EM_GETOLEINTERFACE, 0, (LPARAM)&pEditOle, NULL);
	if (pEditOle)
	{
		for (int i = 0 ; i< pEditOle->GetObjectCount();i++)
		{
			REOBJECT reobject = {0};
			reobject.cbStruct = sizeof(REOBJECT);
			pEditOle->GetObject(i,&reobject,REO_GETOBJ_POLEOBJ);

			if (reobject.poleobj!=NULL)
			{
				IOleInPlaceObjectWindowless* pIoip = NULL;
				if(SUCCEEDED(reobject.poleobj->QueryInterface(IID_IOleInPlaceObjectWindowless,(LPVOID*)&pIoip)))
				{
					pIoip->OnWindowMessage(uMsg, wParam, lParam,NULL);
				}
...
BOOL CRichEditUI::SetStartIndent(int nSize)
{
	PARAFORMAT2 pf2;
	...
	HRESULT lRes = 0;
	TxSendMessage(EM_SETPARAFORMAT, 0, (LPARAM)&pf2, &lRes);
	return (BOOL)lRes;
}
BOOL CRichEditUI::SetLineSpace(int nSpace)
{
	PARAFORMAT2 pf;
	memset(&pf, 0, sizeof(pf));
	pf.cbSize = sizeof(PARAFORMAT2);
	pf.dwMask = PFM_LINESPACING;
	pf.dyLineSpacing = nSpace;
	pf.bLineSpacingRule = 3;
	HRESULT lRes = 0;
	TxSendMessage(EM_SETPARAFORMAT, 0, (LPARAM)&pf, &lRes);
	return (BOOL)lRes;
}
LONG CRichEditUI::GetEditStyle()
{
	PARAFORMAT2 pf;
	memset(&pf, 0, sizeof(pf));
	pf.cbSize = sizeof(PARAFORMAT2);
	HRESULT lRes = 0;
	TxSendMessage(EM_GETPARAFORMAT, 0, (LPARAM)&pf, &lRes);
	return pf.sStyle;
}
void CRichEditUI::SetEditStyle(LONG lStyle)
{
	PARAFORMAT2 pf;
	memset(&pf, 0, sizeof(pf));
	pf.cbSize = sizeof(PARAFORMAT2);
...
void CRichEditUI::ScrollLine(int nLine)
{
	TxSendMessage(EM_LINESCROLL, 0, nLine, 0);
}
} // namespace DuiLib
