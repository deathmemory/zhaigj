...
BOOL CWindowWnd::ShowModal(HWND hParentWindow)
{
	if (!m_hWnd)
	{
...
void CWindowWnd::CenterWindow(HWND hWndC)
{
    ASSERT(::IsWindow(m_hWnd));
	...
	if (rcCenter.left<0&&rcCenter.right<0&&rcCenter.top<0&&rcCenter.bottom<0)
	{
		rcCenter = rcArea;
	}
