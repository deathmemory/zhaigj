...
LRESULT CActiveXUI::MessageHandler(LPMSG lpMsg, BOOL& bHandled)
{
   ...
   // ASSERT(m_pControl->m_bWindowless);
    ...
    LRESULT lResult = 0;
	HRESULT Hr = 0;
	if(lpMsg->hwnd == GetHostWindow()||::IsChild(GetHostWindow(),lpMsg->hwnd) )
		//lResult = SendMessage(lpMsg->hwnd ,WM_FORWARDMSG, 0, (LPARAM)lpMsg);
		// Hr= m_pControl->m_pInPlaceObject->OnWindowMessage(WM_FORWARDMSG, 0, (LPARAM)lpMsg, &lResult);
   if( Hr == S_OK ) bHandled = FALSE;
...
CString CActiveXUI::GetModuleName() const
{
