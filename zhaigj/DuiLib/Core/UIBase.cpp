...
BOOL CNotifyPump::AddVirtualWnd(CDuiString strName,CNotifyPump* pObject)
{
	...
	{
		m_VirtualWndMap.Insert(strName.GetString(),(LPVOID)pObject);
		return TRUE;
	}
	return FALSE;
}
	...
	if( !msg.sVirtualWnd.IsEmpty() ){
		for( int i = 0; i< m_VirtualWndMap.GetSize(); i++ ) {
			if( LPCTSTR key = m_VirtualWndMap.GetAt(i) ) {
				if( _tcsicmp(key, msg.sVirtualWnd.GetString()) == 0 ){
...
// UINT CWindowWnd::ShowModal(HWND hParentWindow)
// {
// 	if (!m_hWnd)
// 	{
// 		Create(hParentWindow,GetWindowClassName(),WS_POPUP|WS_VISIBLE,0,0,0,0,0,0);
// 	}
//     ASSERT(::IsWindow(m_hWnd));
//     UINT nRet = 0;
...
void CWindowWnd::CenterWindow(HWND hCen)
{
    ASSERT(::IsWindow(m_hWnd));
