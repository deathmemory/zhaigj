...
void CRenderEngine::DrawHtmlText(HDC hDC, CPaintManagerUI* pManager, RECT& rc, LPCTSTR pstrTxt, DWORD dwTextColor, RECT* prcLinks, CDuiString* sLinks, int& nLinkRects, UINT uStyle,int nLinkUnder)
{
	...
	while( *pstrText != _T('\0') ) {
				...
				case _T('i'):  // Italic or Image
					{    
						...
						if( sName.IsEmpty() ) { // Italic
							...
							if( _tcsstr(sImageString.GetString(), _T("file=\'")) != NULL || _tcsstr(sImageString.GetString(), _T("res=\'")) != NULL ) {
								CDuiString sImageResType;
								CDuiString sImageName;
								LPCTSTR pStrImage = sImageString.GetString();
								CDuiString sItem;
								CDuiString sValue;
								while( *pStrImage != _T('\0') ) {
									sItem.Empty();
		...
		else
		{
			...
			while( *p != _T('\0') && *p != _T('\n') ) {
				...
				if( pt.x + szText.cx > rc.right ) {
					if( pt.x + szText.cx > rc.right /*&& pt.x != rc.left*/) {      //// by lyb 10.28 部分大字体换行时有字幕显示不出来的问题
						cchChars--;
						cchSize -= (int)(pstrNext - p);
					}
