...
m_pParentResourcePM(NULL)
{
	...
	lf.lfHeight = -12;
	lf.lfWeight = 0;
	StrCpy(lf.lfFaceName,MSYH);
	if (CPaintManagerUI::m_pStrDefaultFontName.GetLength()>0)
	{
		_tcscpy_s(lf.lfFaceName, LF_FACESIZE, CPaintManagerUI::m_pStrDefaultFontName.GetString());
	}
...
BOOL CPaintManagerUI::PreMessageHandler(UINT uMsg, WPARAM wParam, LPARAM lParam, LRESULT& /*lRes*/)
{
    ...
    {
        BOOL bHandled = FALSE;
		MSG Msg;Msg.hwnd = GetPaintWindow();Msg.lParam = lParam;Msg.wParam = wParam;Msg.message = uMsg;
        LRESULT lResult = static_cast<IMessageFilterUI*>(m_aPreMessageFilters[i])->MessageHandler(&Msg, bHandled);
        if( bHandled ) {
            return TRUE;
        }
    }
    ...
    case WM_LBUTTONUP:
        {
            ...
            event.dwTimestamp = ::GetTickCount();
            m_pEventClick->Event(event);
			CControlUI* pControlNowOn = FindControl(pt);
			if (pControlNowOn!=m_pEventClick)
			{
				event.Type = UIEVENT_MOUSELEAVE;
				event.pSender = m_pEventClick;
				m_pEventClick->Event(event);
...
void CPaintManagerUI::HardSetEventClick(CControlUI * pControl)
{
	m_pEventClick = pControl;
}
...
const TImageInfo* CPaintManagerUI::AddImage(LPCTSTR bitmap, LPCTSTR type, DWORD mask)
{
	...
	int nIndexRes = _ttoi(bitmap);
	if (nIndexRes!=0)
	{
		data = CRenderEngine::LoadImage(RES(nIndexRes), _T("PNG"), mask);
		if (!data)
		{
			data = CRenderEngine::LoadImage(RES(nIndexRes), _T("JPG"), mask);
		}
		if (!data)
		{
			data = CRenderEngine::LoadImage(RES(nIndexRes), _T("GIF"), mask);
		}
		if (!data)
		{
			data = CRenderEngine::LoadImage(RES(nIndexRes), _T("TGA"), mask);
		}
	}
    else {
            ...
            if( data != NULL ) {
                if( !data->sResType.IsEmpty() ) {
                    if( isdigit(*bitmap) ) {
                        ...
                        pNewData = CRenderEngine::LoadImage(RES(iIndex), data->sResType.GetString(), data->dwMask);
                    }
                }
                else {
