...
namespace DuiLib
{
	...
	void CContainerUI::SetFloatPos(int iIndex)
	{
		...
		else if (pControl->GetStretchMode()!=0)
		{
			DWORD dwStretch = pControl->GetStretchMode();
			RECT rcStretch = pControl->GetStretchRect();

			if( (dwStretch & UISTRETCH_MOVE_X) != 0 ) 
			{
				int nOffset = (m_rcItem.right - rcCtrl.right) - rcStretch.right;
				::OffsetRect(&rcCtrl, nOffset, 0);
			}
			if( (dwStretch & UISTRETCH_MOVE_Y) != 0 )
			{
				int nOffset = (m_rcItem.bottom - rcCtrl.bottom) - rcStretch.bottom;
				::OffsetRect(&rcCtrl, 0, nOffset);
			}
			if( (dwStretch & UISTRETCH_MOVE_HALF_X) != 0 )
			{
				int nOffset = (m_rcItem.right - rcCtrl.right) - rcStretch.right;
				::OffsetRect(&rcCtrl, nOffset/2, 0);
			}
			if( (dwStretch & UISTRETCH_MOVE_HALF_Y) != 0 )
			{
				int nOffset = (m_rcItem.bottom - rcCtrl.bottom) - rcStretch.bottom;
				::OffsetRect(&rcCtrl, 0, nOffset/2);
			}
			if( (dwStretch & UISTRETCH_SIZE_X) != 0 ) 
			{
				int nOffset = (m_rcItem.right - rcCtrl.right) - rcStretch.right;
				rcCtrl.right += nOffset;
			}
			if( (dwStretch & UISTRETCH_SIZE_Y) != 0 ) 
			{
				int nOffset = (m_rcItem.bottom - rcCtrl.bottom) - rcStretch.bottom;
