...
m_dwTag1(0)
{
	...
	::ZeroMemory(&m_rcStretch, sizeof(RECT));
}
...
void CControlUI::SetFixedPos(RECT rcP)
{
	SetFixedXY(CSize(rcP.left,rcP.top));
	SetFixedWidth(rcP.right - rcP.left);
	SetFixedHeight(rcP.bottom - rcP.top);
}
...
DWORD CControlUI::GetTag1() const //
{
	return m_dwTag1;
}

void CControlUI::SetTag1(DWORD pTag)
{
	m_dwTag1 = pTag;
}
...
{
	if (m_sBkImage == _T("1006"))
	{
		int i = 0;
	}

    if( m_sBkImage.IsEmpty() ) return;
    if( !DrawImage(hDC, (LPCTSTR)m_sBkImage,m_strBkImageModify) ) m_sBkImage.Empty();
}
...
void CControlUI::SetBkImageModify(LPCTSTR pmodify)
{
	m_strBkImageModify = pmodify;
}

void CControlUI::SetID(UINT uID)
{
	m_uID = uID;
}
...
void CControlUI::SetBkImage(UINT uID)
{
	SetBkImage(RES(uID));
}
void CControlUI::SetStretchMode(DWORD dwStret,RECT rc)
{
	m_dwStretchMode = dwStret;
	m_rcStretch = rc;
}
DWORD CControlUI::GetStretchMode()
{
	return m_dwStretchMode;
}
RECT CControlUI::GetStretchRect()
{
	return m_rcStretch;
}
} // namespace DuiLib
