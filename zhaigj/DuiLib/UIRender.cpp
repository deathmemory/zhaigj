#include "StdAfx.h"
#include "../TGAImage/ximage.h"
#include "../UILanguage/UILanguage.h"
///////////////////////////////////////////////////////////////////////////////////////
...
TImageInfo* CRenderEngine::LoadImage(UINT nID, LPCTSTR type, HMODULE hRes)
{
	LPBYTE pData = NULL;
	DWORD dwSize = 0;
	if (type == NULL) type = CPaintManagerUI::GetDefaultResType();
	if (hRes == NULL) hRes = CPaintManagerUI::GetResourceDll();

	if (lstrcmp(type,_T("TGA"))==0)																		
	{
		CxImage tImage;
		HRSRC hrFile = FindResource(hRes,MAKEINTRESOURCE(nID), _T("TGA"));     //后面参数很重要   
		if(hrFile!=NULL) 
		{ 
			tImage.LoadResource(hrFile,CXIMAGE_FORMAT_TGA,hRes);
		}
		HBITMAP s = tImage.MakeNewBitmap();
	//	HBITMAP bmp = CreateDIBSection(NULL,(LPBITMAPINFO)pDib,DIB_RGB_COLORS, &pBit32, NULL, 0);
		//if (pBit32) memcpy(pBit32, GetBits(), head.biSizeImage);

		TImageInfo* data = new TImageInfo;
		data->hBitmap = s;
		data->nX = tImage.GetWidth();
		data->nY = tImage.GetWidth();
		data->alphaChannel = TRUE;
		return data;
	}
	else
	{
		HRSRC hResource = ::FindResource(hRes, MAKEINTRESOURCE(nID), type);
			...
			if (::CreateStreamOnHGlobal(hBuffer, FALSE, &pStream) == S_OK)
			{
				...
				{ 
					if ((pImageTemp)->GetLastStatus() == Gdiplus::Ok)
					{
						...
						x = pImageTemp->GetWidth();
						y = pImageTemp->GetHeight();
						Rect rcImage(0,0, x, y);
						pImageTemp->LockBits(&rcImage, ImageLockModeRead, pImageTemp->GetPixelFormat(), pBitmapData); 
						
						Bitmap * pImage = new Bitmap(pBitmapData->Width, pBitmapData->Height,PixelFormat32bppARGB );

						pImageTemp->UnlockBits(pBitmapData);

						Graphics graph(pImage);

						RectF rfDest(PointF((REAL)0, (REAL)0), SizeF((REAL)x, (REAL)y));
						
						graph.DrawImage(pImageTemp, rfDest, (REAL)0, (REAL)0, (REAL)x, (REAL)y, UnitPixel, NULL);

						pImage->LockBits(&rcImage, ImageLockModeRead, pImage->GetPixelFormat(), pBitmapData);
						LPBYTE byt = (LPBYTE)pBitmapData->Scan0;
						for( int i = 0; i < x * y; i++ ) 
						{
							if( byt[i*4 + 3] < 255 )
							{
...
TImageInfo* CRenderEngine::LoadImage(STRINGorID bitmap, LPCTSTR type, DWORD mask)
{
    ...
    if( type == NULL ) {
        CString sFile = CPaintManagerUI::GetResourcePath();
        if( CPaintManagerUI::GetResourceZip().IsEmpty() ) {
            sFile += bitmap.m_lpstr;
            HANDLE hFile = ::CreateFile(sFile.GetString(), GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, \
            ...
            HZIP hz = OpenZip((void*)sFile.GetString(), 0, 2);
            ZIPENTRY ze; 
            int i; 
                                    ...
                                    BYTE uFade, BOOL hole, BOOL xtiled, BOOL ytiled)
{
	...
	::SetStretchBltMode(hCloneDC, HALFTONE);
    RECT rcTemp = {0};
    RECT rcDest = {0};
...
BOOL DrawImage(HDC hDC, CPaintManagerUI* pManager, const RECT& rc, const RECT& rcPaint, const CString& sImageName, const UINT& nImageID,\
		const CString& sImageResType, RECT rcItem, RECT rcBmpPart, RECT rcCorner, DWORD dwMask, BYTE bFade, \
		BOOL bHole, BOOL bTiledX, BOOL bTiledY,BOOL BmpPartPresent = FALSE)
{
	const TImageInfo* data = NULL;
	CString sType = sImageResType;
	...
	else
	{
		if (sType.IsEmpty())
		{
			sType = CString(CPaintManagerUI::GetDefaultResType());
		}
		data = pManager->GetImageEx(nImageID, (LPCTSTR)sType, dwMask);
	}
	...
	if (BmpPartPresent)
	{
		...
		RECT rcPersent = rcBmpPart;
		rcBmpPart.left = rcPersent.left*data->nX/100;
		rcBmpPart.right = rcPersent.right*data->nX/100;
		rcBmpPart.top = rcPersent.top*data->nY/100;
		rcBmpPart.bottom = rcPersent.bottom*data->nY/100;
	}
	else
	{
		if( rcBmpPart.left == 0 && rcBmpPart.right == 0 && rcBmpPart.top == 0 && rcBmpPart.bottom == 0 ) {
...
BOOL CRenderEngine::DrawImageString(HDC hDC, CPaintManagerUI* pManager, const RECT& rc, const RECT& rcPaint, 
                                          LPCTSTR pStrImage,UINT nID, LPCTSTR pStrModify)
{
    ...
    BOOL bTiledX = FALSE;
    BOOL bTiledY = FALSE;
	BOOL bBmpPartPersent = FALSE;
	if (pStrImage)sImageName = pStrImage;
        ...
        while( *pStrImage != _T('\0') ) {
            ...
            if( !sValue.IsEmpty() ) {
                if( sItem == _T("file") || sItem == _T("res") ) {
					if( image_count > 0 )
						DuiLib::DrawImage(hDC, pManager, rc, rcPaint, sImageName,nID, sImageResType,
                    ...
                    rcItem.left = rc.left + _tcstol(sValue.GetString(), &pstr, 10);  ASSERT(pstr);    
                    ...
                    rcBmpPart.left = _tcstol(sValue.GetString(), &pstr, 10);  ASSERT(pstr);    
                    ...
                    rcCorner.left = _tcstol(sValue.GetString(), &pstr, 10);  ASSERT(pstr);    
                    rcCorner.top = _tcstol(pstr + 1, &pstr, 10);    ASSERT(pstr);    
                    rcCorner.right = _tcstol(pstr + 1, &pstr, 10);  ASSERT(pstr);    
                    rcCorner.bottom = _tcstol(pstr + 1, &pstr, 10); ASSERT(pstr);
                }
                else if( sItem == _T("mask") ) {
                    if( sValue[0] == _T('#')) dwMask = _tcstoul(sValue.GetString() + 1, &pstr, 16);
                    else dwMask = _tcstoul(sValue.GetString(), &pstr, 16);
                }
                else if( sItem == _T("fade") ) {
                    bFade = (BYTE)_tcstoul(sValue.GetString(), &pstr, 10);
                }
                else if( sItem == _T("hole") ) {
                    bHole = (_tcscmp(sValue.GetString(), _T("TRUE")) == 0);
                }
                else if( sItem == _T("xtiled") ) {
                    bTiledX = (_tcscmp(sValue.GetString(), _T("TRUE")) == 0);
                }
                else if( sItem == _T("ytiled") ) {
                    bTiledY = (_tcscmp(sValue.GetString(), _T("TRUE")) == 0);
                }
				else if( sItem == _T("sourcePersent") ) {
					bBmpPartPersent = (_tcscmp(sValue.GetString(), _T("TRUE")) == 0);
				}
            }
            if( *pStrImage++ != _T(' ') ) break;
        }
	...
	DuiLib::DrawImage(hDC, pManager, rc, rcPaint, sImageName,nID, sImageResType,
		rcItem, rcBmpPart, rcCorner, dwMask, bFade, bHole, bTiledX, bTiledY,bBmpPartPersent);
...
void CRenderEngine::DrawLine(HDC hDC, const RECT& rc, int nSize, DWORD dwPenColor)
{
	...
	Graphics gr(hDC);
	Gdiplus::Pen LinePen(Color(GetBValue(dwPenColor), GetGValue(dwPenColor), GetRValue(dwPenColor)));LinePen.SetWidth((REAL)nSize);
	
	gr.SetSmoothingMode(SmoothingMode::SmoothingModeHighQuality);
	gr.DrawLine(&LinePen, Point(rc.left, rc.top),Point(rc.right, rc.bottom));
 ...
 //   ::LineTo(hDC, rc.right, rc.bottom);
	//CPoint po[2] = { CPoint(rc.left, rc.top),CPoint(rc.right, rc.bottom)};
	//::Polyline( hDC,po,2);
...
void CRenderEngine::DrawText(HDC hDC, CPaintManagerUI* pManager, RECT& rc, LPCTSTR pstrTxt, DWORD dwTextColor, int iFont, UINT uStyle)
{
    ASSERT(::GetObjectType(hDC)==OBJ_DC || ::GetObjectType(hDC)==OBJ_MEMDC);
	CString strLanguage(CUILanguage::ConvertLanguage(pstrTxt,-1));
	LPCTSTR pstrText = (LPCTSTR)strLanguage;
...
void CRenderEngine::DrawHtmlText(HDC hDC, CPaintManagerUI* pManager, RECT& rc, LPCTSTR pstrTxt, DWORD dwTextColor, RECT* prcLinks, CString* sLinks, int& nLinkRects, UINT uStyle,int nLinkUnder)
{
        ...
        if( pt.x >= rc.right || *pstrText == _T('\n') || bLineEnd ) {
            ...
            if( !bLineDraw ) {
                if( bInLink && iLinkIndex < nLinkRects ) {
                    ...
                    CString *pStr1 = (CString*)(sLinks + iLinkIndex - 1);
                    CString *pStr2 = (CString*)(sLinks + iLinkIndex);
                    *pStr2 = *pStr1;
                }
            ...
            case _T('a'):  // Link
                {
                    ...
                    if( iLinkIndex < nLinkRects && !bLineDraw ) {
                        CString *pStr = (CString*)(sLinks + iLinkIndex);
                        pStr->Empty();
                    ...
                    if( pFontInfo->bUnderline == FALSE ) {
						HFONT hFont = pManager->GetFont(pFontInfo->sFontName, pFontInfo->iSize, pFontInfo->bBold,(nLinkUnder!=-1)? TRUE:FALSE, pFontInfo->bItalic);
                        if( hFont == NULL ) hFont = pManager->AddFont(pFontInfo->sFontName, pFontInfo->iSize, pFontInfo->bBold, (nLinkUnder!=-1) ?TRUE:FALSE, pFontInfo->bItalic);
            ...
            case _T('i'):  // Italic or Image
                {
                    ...
                    if( sName.IsEmpty() ) { // Italic
                        ...
                        pImageInfo = pManager->GetImage((LPCTSTR)sName);
                        if( pImageInfo ) {
