...
void CContainerUI::SetLineSize(int nSize)
{
	m_nLineSize = nSize;
}
...
void CContainerUI::SetPos(RECT rc)
{
	...
	//DUITRACE(_T("%d,%d,%d,%d"),m_rcItem.left,m_rcItem.top,m_rcItem.right,m_rcItem.bottom);
}
...
void CContainerUI::ProcessScrollBar(RECT rc, int cxRequired, int cyRequired)
{
    ...
    if( m_pVerticalScrollBar == NULL && m_pHorizontalScrollBar == NULL) return;

    if( m_pVerticalScrollBar&&cyRequired > rc.bottom - rc.top && !m_pVerticalScrollBar->IsVisible() ) {
	...
	if(m_pHorizontalScrollBar&& cxRequired > rc.right - rc.left && !m_pHorizontalScrollBar->IsVisible() ) {
...
void CDialogLayoutUI::SetMinRect(RECT rc)
{
	m_rcMinRoot = rc;
}
...
void CDialogLayoutUI::SetPos(RECT rc)
{
    ...
    for( int i = 0; i < m_aModes.GetSize(); i++ ) {
		...
		if (!::IsRectEmpty(&pItem->pControl->GetStretchPos()))
		{
			rcPos = pItem->pControl->GetStretchPos();
		}
		RECT rcCurPos = pItem->pControl->GetPos();
		if (rcCurPos.left == rcCurPos.right||rcCurPos.top == rcCurPos.bottom)
		{
			continue;
		}
        int iScrollPosY = 0;
		...
		if( (pItem->uMode & UISTRETCH_MOVE_HALF_X) != 0 ) ::OffsetRect(&rcPos, cxMove/2, 0);
		if( (pItem->uMode & UISTRETCH_MOVE_HALF_Y) != 0 ) ::OffsetRect(&rcPos, 0, cyMove/2);
...
void CDialogLayoutUI::SetCorner(RECT rc)
{
	m_rcCorner = rc;
}
void CDialogLayoutUI::PaintBkImage(HDC hDC)
{
	CString strModify;
	strModify.Format(_T("corner='%d,%d,%d,%d'"),m_rcCorner.left,m_rcCorner.top,m_rcCorner.right,m_rcCorner.bottom);
	...
	if( !DrawImage(hDC, (LPCTSTR)m_sBkImage ,strModify)) m_sBkImage.Empty();
}
/////////////////////////////////////////////////////////////////////////////////////
//
//
