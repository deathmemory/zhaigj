...
CString CRichEditUI::GetText() const
{
...
CString CRichEditUI::GetSelText() const
{
    if( !m_pTwh ) return CString();
    CHARRANGE cr;
    cr.cpMin = cr.cpMax = 0;
    TxSendMessage(EM_EXGETSEL, 0, (LPARAM)&cr, 0);
    LPWSTR lpText = NULL;
...
CString CRichEditUI::GetTextRange(long nStartChar, long nEndChar) const
{
    TEXTRANGEW tr = { 0 };
...
CString CRichEditUI::GetLine(int nIndex, int nMaxLength) const
{
    LPWSTR lpText = NULL;
...
HRESULT CRichEditUI::TxSendMessage(UINT msg, WPARAM wparam, LPARAM lparam, LRESULT *plresult) const
{
    if( m_pTwh ) {
        ...
        HRESULT hRes =  m_pTwh->GetTextServices()->TxSendMessage(msg, wparam, lparam, plresult);
		if (msg == WM_CHAR)
		{
        ...
        case WM_RBUTTONUP:
            {
                ...
                if(pHover != this&&GetParent()!=pHover) {
                    bWasHandled = FALSE;
                    return 0;
                }
            }
