...
m_bFirstSetPos(FALSE)
{
	...
	::ZeroMemory(&m_rcItemStretch, sizeof(RECT));
...
CString CControlUI::GetName() const
{
    return m_sName;
}
...
CString CControlUI::GetText() const
{
    return m_sText;
}
...
void CControlUI::SetTimer(UINT nID , UINT nElapse)
{
	if (m_pManager)
	{
		m_pManager->SetTimer(this,nID,nElapse);
	}
}

void CControlUI::KillTimer(UINT nID)
{
	if (m_pManager)
	{
		m_pManager->KillTimer(this,nID);
	}
}
...
void CControlUI::SetID(UINT nID)
{
	m_nControlID = nID;
}

BOOL CControlUI::IsParentNotify() const
{
	return m_bNotifyParent;
}
void CControlUI::SetParentNotify(BOOL bN)
{
	m_bNotifyParent = bN;
}

BOOL CControlUI::IsChecked() const
{
	return m_bIsChecked;
}
void CControlUI::SetCheck(BOOL bCheck)
{
	m_bIsChecked = bCheck;
	Invalidate();
}
...
void CControlUI::SetBkImage(UINT nImage)
{
	if( m_nBkImage == nImage ) return;

	m_nBkImage = nImage;
	Invalidate();
}

void CControlUI::SetBkImageModify(LPCTSTR pStrImage)
{
	m_sBkImageModify = pStrImage;
	Invalidate();
}

CString CControlUI::GetBkImageModify()
{
	return m_sBkImageModify;
}

void CControlUI::SetResType(LPCTSTR str)
{
	m_sResType = str;
}

CString  CControlUI::GetResType()
{
	return m_sResType;
}
...
{
    return CRenderEngine::DrawImageString(hDC, m_pManager, m_rcItem, m_rcPaint, pStrImage,0, pStrModify);
}

BOOL CControlUI::DrawImage(HDC hDC, UINT nID, LPCTSTR pStrModify)
{
	 return CRenderEngine::DrawImageString(hDC, m_pManager, m_rcItem, m_rcPaint, NULL,nID, pStrModify);
}

RECT CControlUI::GetStretchPos() const
{
	return m_rcItemStretch;
}

void CControlUI::SetStretchPos(RECT rcPos)
{
	m_rcItemStretch = rcPos;
}
...
void CControlUI::SetPos(RECT rc)
{
	...
	//m_cXY = CSize(rc.left,rc.top);

	if (m_cxyFixed.cx == 0 && m_cxyFixed.cy == 0)
	{
		m_cxyFixed = CSize(rc.right - rc.left,rc.bottom - rc.top);
	}
    if( m_bFloat ) {
...
CString CControlUI::GetToolTip() const
{
    return m_sToolTip;
}
...
const CString& CControlUI::GetUserData()
{
    return m_sUserData;
}
...
void CControlUI::PaintBkImage(HDC hDC)
{
	...
	{
		if( !DrawImage(hDC, (LPCTSTR)m_sBkImage,m_sBkImageModify) ) m_sBkImage.Empty();
	}
	else if (m_nBkImage!=0)
	{
		CString strModify;
		strModify.Format(_T("%s restype='%s'"),(LPCTSTR)m_sBkImageModify,(LPCTSTR)m_sResType);
...
BOOL CControlUI::GetMouseMoveNotify() const
{
	return m_bMouseMoveNotify;
}

void CControlUI::SetMouseMoveNotify(BOOL bNotify)
{
	m_bMouseMoveNotify = bNotify;
}
} // namespace DuiLib
